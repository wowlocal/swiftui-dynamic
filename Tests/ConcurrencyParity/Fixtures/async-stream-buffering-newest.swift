final class AsyncStreamBufferingNewestState: @unchecked Sendable {
    var yieldResults: [String] = []
}

@MainActor
func asyncStreamBufferingNewestProbe() async -> String {
    let state = AsyncStreamBufferingNewestState()
    let stream = AsyncStream<Int>(bufferingPolicy: .bufferingNewest(2)) {
        continuation in
