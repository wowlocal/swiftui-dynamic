...
struct AsyncThrowingStreamRuntimeTests {
    @Test func unverifiedFailureCallbackAndBufferingFailClosed() async throws {
        let sources = [
            (source: """
                enum ProbeFailure: Error { case stopped }
                let stream = AsyncThrowingStream<Int, Error> { continuation in
                    continuation.onTermination = { _ in }
                    continuation.finish(throwing: ProbeFailure.stopped)
                }
                """, diagnostic:
