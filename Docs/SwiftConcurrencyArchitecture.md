...
value. When no child is cancelled, explicit throwing `waitForAll` consumes the
same completion-ordered queue as `next`. It retains the first failed source
outcome, continues draining every remaining outcome, returns normally if none
failed, and otherwise rethrows that first error only after the drain completes.
Outcomes already consumed by an earlier `next` are not reconsidered.
Cancellation projection and exceptional-exit rules remain explicitly
unsupported until established by dedicated native probes.
