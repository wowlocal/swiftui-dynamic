...
| M5 actor support and executor architecture | partial | Logical cooperative-default and MainActor executor identity, source hops, caller restoration, detached-task lane identity, distinct source-actor runtime IDs, weak actor-record cleanup, and isolated instance-method entry with caller restoration are covered. | Active demand cycle (2026-07-16): complete actor-owned storage enforcement, serial mailbox/executor queues, and user-declared global-actor mapping (FoodTruck StoreActor shape), then reentrancy/resume ownership, isolated parameters, arbitrary global actors, and mailbox stress, then per-feature fail-closed flips of the class-like compatibility path with no up-front global rejection; physical workers remain M9. |
...
This is deliberately not a claim of actor storage isolation, a serial mailbox,
reentrancy, user-defined global actors, or physical parallelism. Those remain
open in the M5 acceptance requirement and gap ledger.
