...
operation/scheduler registries. The declaration remains a known divergence:
throwing MainActor evidence, omitted and arbitrary-source-actor isolation,
`resume(with:)`, double-resume and abandonment policy, escaped-token lifetime,
and unsafe continuations remain open.
...
Focused evidence records two created records, two canonical suspensions, exact
value/error projection, and empty continuation/task/scope/group/stream/host-
operation/scheduler registries. Nonthrowing `Result<T, Never>` and
existential-error `Result<T, any Error>` spellings, throwing MainActor
isolation, omitted and arbitrary-source-actor isolation, diagnostics/lifetime,
and unsafe continuations remain open.
...
scheduler registries. Nonthrowing and existential-error Result resume
spellings, throwing MainActor evidence, omitted and arbitrary-source-actor
isolation, double-resume and abandonment policy, escaped-token lifetime, and
unsafe continuations remain open.
