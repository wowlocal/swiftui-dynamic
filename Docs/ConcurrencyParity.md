...
| M8 SwiftUI lifecycle | partial | Synchronous callbacks use runtime-owned host tasks. BridgeGen emits `.task`, `.task(id:)`, and `.refreshable`; one reusable adapter gives actual SwiftUI-owned invocations fresh `.swiftUITask` sessions. Same-source hosting probes cover async entry, MainActor identity, disappearance cancellation, id replacement, and cleanup. | Characterize repeated same-id rendering and `.refreshable` completion lifetime, then add teardown stress and retained-session release evidence. |
...
Three source-identical native/interpreted fixtures use actual `NSHostingView`
...
- changing `.task(id:)` from `1` to `2` cancels the first task, starts the
  ...
  between the old cancellation handler and the new task start.

The combined strict Swift 6 native observation was stable in all twenty runs:
`entry=started,value:21|disappearance=started,cancelled|id=cancel:1,cancel:2,start:1,start:2`,
with SHA-256
`661a004328001dd4a340fbbaf7adb441b653659adde6c0d08953aba3a5a0381e`.
...
M8 remains partial. Re-rendering the same identity/id without a restart,
`.refreshable` trigger/completion lifetime, repeated teardown stress, and weak
retained-session release evidence are still open. Physical threads remain M9.
