...
| M6 async sequences/continuations | partial | Protocol-level `for await` dispatches `makeAsyncIterator()` and suspending `next()` witnesses through the ordinary evaluator/executor path for interpreted values, protocol-extension defaults, and typed opaque host gateways. Exact same-source parity covers finite success, typed source failure, cooperative user-iterator cancellation, `break`/`continue`/`return` plus per-iteration and function-level `defer` cleanup, real source and host suspension, iterator state, terminal `nil`, host-operation ownership, and registry cleanup. Unbounded `AsyncStream` now has runtime-owned empty-stream suspension, producer yield/finish, synchronous one-shot `.finished` and `.cancelled` callbacks, cancellation-before-terminal-`nil`, stable repeated terminal reads, rejected post-finish yield, independent and copied iterators parked concurrently on shared storage, final-reference scope-exit cancellation, and complete record cleanup. Task-group iteration remains separate M4 evidence. | Complete escaped-producer `AsyncStream` lifetime and buffering policies; then implement `AsyncThrowingStream` and checked continuations on cooperative-default and MainActor executors with error, double-resume, abandonment, lifetime, and cleanup evidence. |
...
Generated-surface and capability-accounting checks retain 171 declarations,
123 routes, and 167 reviewed rows: 51 runtime-supported, 20
...
unsafe value/error types, async/throwing/sending/isolation metadata, and the
absence of adapter routes. Every row is explicitly `deferred`/`none` under
...
Accounting is now 171/171 reviewed: 51 runtime-supported, 20
...
registries after completion. Escaped-producer lifetime, bounded buffering,
`AsyncThrowingStream`, and source continuations remain open.
