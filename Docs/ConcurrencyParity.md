...
| M9 physical parallelism | partial | The core remains cooperatively scheduled and main-actor hosted; no physical parallelism claim is made. Parsing and operator folding produce an immutable Sendable ParsedProgram reusable by detached readers and independent interpreter sessions without shared evaluator state. Each Interpreter owns a distinct MainActor-confined RuntimeHeap that roots its actual global environment, synthesized environment models, and SwiftUI state cells. Every runAsync program entry executes through a single-use InterpreterSession binding the program, heap, cooperative runtime, runtime ID, lazy-global mode, and completion policy; ownership, identity, rejection, draining, and lifetime are covered. The source-callable _swift_createJobForTestingOnly hook remains preflight-only/deferred. | Move declaration metadata and evaluation fully behind InterpreterSession, unify raw-ID host/SwiftUI callback entries, define or reject overlapping facade sessions, classify every heap edge for worker access, add physical-worker scheduling, Thread Sanitizer evidence, and cooperative-versus-parallel semantic parity without exposing Box, Environment, or Instance to races. |
