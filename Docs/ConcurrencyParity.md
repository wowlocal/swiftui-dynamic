...
| M8 SwiftUI lifecycle | partial | Synchronous callbacks use runtime-owned host tasks. BridgeGen emits `.task`, `.task(id:)`, and `.refreshable`; one reusable adapter gives actual SwiftUI-owned invocations fresh `.swiftUITask` sessions. Same-source hosting probes cover async entry, MainActor identity, disappearance cancellation, id replacement, same-id preservation, refresh trigger/completion, and cleanup. | Add repeated teardown stress and retained-session release evidence. |
...
Five source-identical native/interpreted fixtures use actual `NSHostingView`
...
- the public `RefreshAction` installed by `.refreshable` does not return before
  ...
  source gate, and then observes `finished,returned` in that causal order.
...
`entry=started,value:21|disappearance=started,cancelled|id=cancel:1,cancel:2,start:1,start:2|same-id=finish:first,render:first,render:same,start:first|refreshable=started,finished,returned`,
with SHA-256
`8de420526ffc88b30f65357a81e67193942608560dd624a0a96265ab06b1b31a`.
...
This same-id step was an already-GREEN characterization and required no
production change: real SwiftUI continues to own modifier identity while the
adapter remains scoped to one actual invocation. M8 remains partial for
repeated teardown stress and weak retained-session release evidence. Physical
threads remain M9.
