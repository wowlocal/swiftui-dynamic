...
Four source-identical native/interpreted fixtures use actual `NSHostingView`
...
- re-rendering a changed view value with the same `.task(id:)` preserves the
  ...
  a scheduler-order assertion.

The combined strict Swift 6 native observation was stable in all twenty runs:
`entry=started,value:21|disappearance=started,cancelled|id=cancel:1,cancel:2,start:1,start:2|same-id=finish:first,render:first,render:same,start:first`,
with SHA-256
`40c3e91ca37f70606edb393ee1d57f05b4318148eff5a9ce6bdc62c54e45faff`.
...
`.refreshable` trigger/completion lifetime, repeated teardown stress, and weak
retained-session release evidence. Physical threads remain M9.
