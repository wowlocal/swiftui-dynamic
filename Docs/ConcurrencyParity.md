...
| M6 async sequences/continuations | partial | Protocol-level `for await` now dispatches `makeAsyncIterator()` and suspending mutating `next()` witnesses through the ordinary evaluator/executor path. Exact same-source parity covers a finite nonthrowing interpreted sequence, real suspension, iterator value-state copy-out, terminal `nil`, and registry cleanup. Task-group iteration remains separate M4 evidence. | Add throwing, cancellation, early-exit, protocol-default, and host-bridged sequence coverage; then implement `AsyncStream`/`AsyncThrowingStream` and checked continuations on cooperative-default and MainActor executors with termination, double-resume, abandonment, lifetime, and cleanup evidence. |
...
registries. M6 is therefore `partial`, not complete: throwing iteration,
cancellation, `break`/`return` cleanup, protocol-extension defaults,
host-bridged sequences, streams, continuation ownership, and executor-correct
resume remain in the active requirement.
