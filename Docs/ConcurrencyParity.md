...
| M6 async sequences/continuations | partial | Protocol-level `for await` dispatches `makeAsyncIterator()` and suspending `next()` witnesses through the ordinary evaluator/executor path for interpreted values, protocol-extension defaults, and typed opaque host gateways. Exact same-source parity covers finite success, typed source failure, cooperative user-iterator cancellation, `break`/`continue`/`return` plus per-iteration and function-level `defer` cleanup, real source and host suspension, iterator state, terminal `nil`, host-operation ownership, and registry cleanup. The first unbounded `AsyncStream` slice adds runtime-owned empty-stream suspension, two producer yields, finish-to-`nil`, and complete record cleanup. Task-group iteration remains separate M4 evidence. | Complete `AsyncStream` cancellation, termination callbacks, producer/consumer lifetime, iterator copies, and buffering policies; then implement `AsyncThrowingStream` and checked continuations on cooperative-default and MainActor executors with error, double-resume, abandonment, lifetime, and cleanup evidence. |
