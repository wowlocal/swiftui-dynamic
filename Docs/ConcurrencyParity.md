...
| M9 physical parallelism | partial | The core remains cooperatively scheduled and main-actor hosted; no physical parallelism claim is made. Parsing and operator folding now produce an immutable Sendable ParsedProgram that is reusable by detached readers and independent interpreter sessions without shared evaluator state. The source-callable _swift_createJobForTestingOnly hook remains preflight-only/deferred. | Move declaration metadata into the immutable program boundary, separate the session/runtime heap, add worker synchronization or confinement, Thread Sanitizer evidence, and cooperative-versus-parallel semantic parity without exposing Box, Environment, or Instance to races. |
