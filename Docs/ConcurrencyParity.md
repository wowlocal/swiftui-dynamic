...
| `parallel-detached-file-manager-move` | exact plus physical receipt | Awaited `Task.detached` operations may synchronously move and copy Foundation URLs with `FileManager.default`; `Task.value` returns only after each file-system effect is visible, and a subsequent move from the missing source throws into the caller's do/catch | Parallel-mode parity preserves `worker\|source:false\|destination:true\|copy:true\|missing:error` with three physical executions from four submissions (the failing missing-source move drains its permit and record); the moveItem/copyItem table rows serve both faces from one prepared kernel and cooperative mode records zero physical receipts |
| `parallel-detached-file-manager-remove` | exact plus physical receipt | Awaited `Task.detached` operations may synchronously remove Foundation URL and path String arguments with `FileManager.default`; `Task.value` returns only after each removal is visible, and removing the same missing URL throws into the caller's do/catch | Parallel-mode parity preserves `url:true\|path:true\|missing:error` with two physical executions from three submissions; one removeItem row merges the URL and path spellings exactly as the pre-migration hand gateway did |
| `parallel-detached-file-manager-exists` | exact plus physical receipt | Awaited `Task.detached` operations may synchronously query `FileManager.default.fileExists(atPath:)`; `Task.value` publishes true for an existing path and false for a missing path after each read completes | Parallel-mode parity preserves `present:true\|missing:false` with two physical executions; the non-throwing query passes the stated sandbox policy unrestricted, and a degenerate argument shape takes the row's immediate lenient false without submitting a job |
| `parallel-detached-file-manager-create-directory` | exact plus physical receipt | Awaited `Task.detached` operations may synchronously create Foundation URL and path String directories with intermediate directories enabled; `Task.value` returns only after each directory is visible | Parallel-mode parity preserves `url:true\|path:true` with two physical executions; the row preserves the box's inert-accept and always-with-intermediates lenience identically on both faces, and every accepted argument shape shares it |
| `parallel-detached-file-manager-list` | exact plus physical receipt | An awaited `Task.detached` operation may synchronously list a directory with `FileManager.default.contentsOfDirectory(at:includingPropertiesForKeys:)`; `Task.value` publishes the child URLs after the read completes, and `lastPathComponent` projects their names | Parallel-mode parity preserves `alpha\|beta\|gamma` with one physical execution; the generality witness — contentsOfDirectory never had a hand-written worker gateway, its table row plus the shared engine provide the physical face, and the URL-array result crosses through the widened checked value vocabulary |
| `parallel-detached-file-manager-path-forms` | exact plus physical receipt | Awaited `Task.detached` operations may synchronously copy and move items through `FileManager.default`'s atPath:/toPath: String spellings and list child NAMES with `contentsOfDirectory(atPath:)`; the atPath list overload returns String names rather than URLs | Parallel-mode parity preserves `copied:true\|moved:true\|names:inner` with three physical executions; the String spellings are row data merging onto the existing URL kernels (ControlRoom/CodeEdit cite the transfers; blink/CopilotForXcode/Pearcleaner cite the atPath list), with the native names-not-URLs shape carried by the row |
...
| `continuation-entry-points` | exact native characterization | All four public checked/unsafe continuation entry points accept explicit `MainActor.shared` isolation from a nonisolated caller; each inline body runs on MainActor and a one-shot resume returns its exact value | Native 20/20: `checked:11\|checkedThrowing:22\|unsafe:33\|unsafeThrowing:44`; all four public forms now route through generated continuation-shape intrinsics with separate checked/unsafe ownership policies |
| `unsafe-continuation-value-resume` | exact | Explicit `MainActor.shared` isolation runs a nonthrowing unsafe-continuation body on MainActor and immediate one-shot returning resume produces `value:33` | Native/interpreter parity in 20 repetitions; four native five-run shards retain SHA-256 `c5db7f0cebe2ae8a9f5b5a4e14556a8d895ee37ffe163501adbdef9514f301fe`, and focused evidence drains the shared runtime-owned continuation record |
| `unsafe-throwing-continuation-value-resume` | exact | Explicit `MainActor.shared` isolation runs a throwing unsafe-continuation body on MainActor and immediate one-shot returning resume produces `value:44` | Native/interpreter parity in 20 repetitions; four native five-run shards retain SHA-256 `bf29fe0e99e36963d041e8c94108c5b863250595e96a6fc4360e9b86da469d11`, with the generated throwing effect enabling error-capable resume |
| `unsafe-continuation-result-resume` | exact | A throwing unsafe token may escape its synchronous body into a detached producer and later resume its parked owner through Result value, exact source error, or Void success | Native/interpreter parity in 20 repetitions: `value:29\|error:failed\|void:resumed`; every native five-run shard retains SHA-256 `307eb4047d16781730fa928afa9d4af3e419e1612167f2560e156958f2f9bb6d`, and focused evidence records three canonical suspensions plus complete cleanup |
...
### M6 unsafe-continuation demand reopening

The earlier fail-closed boundary was reopened when the expanded OSS corpus
measured 14 files using `withUnsafeContinuation` and 34 using
`withUnsafeThrowingContinuation`. IceCubes supplied the immediate integration
failure: Nuke's `ImagePipeline` creates a task with
`withUnsafeThrowingContinuation`, escapes its token into a labeled
`DispatchQueue`, and later completes it with `resume(with:)`. The captured RED
was the old unsupported-ownership diagnostic before Nuke could submit that
queue closure.

The generator now recognizes the public continuation entry-point property—an
isolation-bearing async declaration whose synchronous body consumes a checked
or unsafe continuation carrier. Both unsafe spellings therefore share one
`unsafeContinuation` intrinsic without adding one runtime branch per API name;
the declaration's generated throwing effect selects error-capable resume. The
existing runtime-owned continuation record supplies suspension, executor
restoration, copied value/error projection, infrastructure teardown, and
bounded cleanup. Its unsafe token policy deliberately omits checked
abandonment warnings. Undefined unsafe double-resume and abandonment behavior
is not claimed.

Immediate MainActor value fixtures pass 20/20 native/interpreter repetitions;
their four five-run native shards retain SHA-256
`c5db7f0cebe2ae8a9f5b5a4e14556a8d895ee37ffe163501adbdef9514f301fe`
and
`bf29fe0e99e36963d041e8c94108c5b863250595e96a6fc4360e9b86da469d11`.
The Nuke-shaped delayed Result fixture also passes 20/20 with exact
`value:29|error:failed|void:resumed`; every native shard retains SHA-256
`307eb4047d16781730fa928afa9d4af3e419e1612167f2560e156958f2f9bb6d`.
Focused evidence records three real `waitingForContinuation` suspensions and
complete continuation/task/session cleanup.
