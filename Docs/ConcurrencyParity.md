...
actual opt-in `RuntimePhysicalWorkerDriver`. Its batch is stateless and bounded:
each active slot launches one `Task.detached`, a throwing task group admits the
next job only after a slot completes, successful results are restored to input
order, and cancellation of a structured slot explicitly cancels its detached
native task. A failure cancels siblings and task-group scope exit drains them.
Invalid bounds throw a typed error, empty batches complete immediately, and a
finished batch retains neither jobs nor operation captures.
