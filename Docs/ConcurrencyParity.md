...
| M9 physical parallelism | partial | M9 remains partial: the general source evaluator is cooperatively scheduled and MainActor-confined, while explicit parallel mode admits only the demand-cited snapshot kernels, continuation prefixes, source-call wrappers, and checked source-synchronous host operations owned by the parallel-runtime-and-sanitizers requirement. ParsedProgram metadata, resolved plans, RuntimeEntry worker capabilities, and copied value snapshots are immutable and Sendable; mutable program state, RuntimeValue graphs, heaps, evaluators, registries, closures, and sandbox ownership remain confined. One bounded Task.detached driver with a shared FIFO permit pool preserves ordered results, cancellation and failure drain, capture release, actor non-reentrancy, and interpreted-trap containment. The checked host subset covers Thread.isMainThread and direct FileManager.default move/copy/remove/fileExists plus the cited createDirectory URL/path spellings. The ninety-seventh slice admits directory creation only for exactly two evaluated arguments with withIntermediateDirectories true and omitted attributes; source shadows, aliases, false, explicit attributes, cooperative execution, and actor-confined execution remain on the ordinary evaluator. Apple Swift 6.3.3 returned exact url:true and path:true in twenty bounded strict runs; the receipt RED was zero/zero instead of two/two, focused parity passed 20/20 after the fix, and the exact-tip physical and fresh TSan boards passed 126/126 tests with native overlap 20/20. | Complete general evaluator and heap parallelism without exposing confined runtime objects. Extend exact source and host call-target resolution beyond the currently demand-admitted core Task, String, Array, Thread, and FileManager subsets; retain source precedence and fail closed for ambiguous or richer shapes. Finish session ownership of mutable symbol materialization and evaluation, support further demand-cited captures, values, suspensions, actor routes, custom executors, and ExecutorJob scheduling, and expand cooperative-versus-parallel differential and Thread Sanitizer coverage for every new physical kernel. M9 stays open until those requirements are evidenced; cooperative mode remains the default. |
...
### Checked detached FileManager directory creation

The ninety-seventh M9 iteration follows a new exact census of the current
94-project corpus. The tree now contains 515 Swift files with `Task.detached`,
and seven inspected detached closure bodies call
`FileManager.default.createDirectory` across Clop, Provenance, session-ios,
and Whisky. Five cite `at: URL`; two cite `atPath: String`; all enable
intermediate directories and omit attributes.

Apple Swift 6.3.3 compiled the same-source fixture in complete-strict Swift 6
mode with warnings as errors. Twenty bounded native runs returned exact
`url:true|path:true`; raw SHA-256 was
...
The interpreter was already behaviorally exact, but explicit parallel mode
recorded zero physical submissions/executions instead of two/two.

The existing checked Void gateway now captures one validated immutable URL and
performs the native creation on the bounded worker. Direct imported-static
identity and the exact cited two-argument shape are mandatory. Source shadows,
aliases, false, explicit attributes, actor-confined work, and cooperative mode
remain confined with zero receipts. The exact-tip physical and fresh TSan
boards passed 126/126 tests in four suites; the sanitizer board also passed
native overlap 20/20 in 31 seconds without diagnostics. The canonical focused
iteration passed those 126 implementation tests, all 46 methodology/gate
checks, and 20/20 parity repetitions in seven seconds. This closes only the
cited directory-creation spellings, not general FileManager forwarding.
