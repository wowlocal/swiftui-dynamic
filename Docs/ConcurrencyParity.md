...
through the existing caller-lexical-isolation mechanism. It then reuses the
same optional-nil/MainActor validation and contextual-executor invocation as
an explicit value. This is not an omission-to-nil shortcut: focused negative
evidence invokes the API from an ordinary source actor and proves that its
actor capability reaches the existing fail-closed boundary before any
...
Arbitrary-source-actor execution, double-resume and abandonment diagnostics,
escaped-token lifetime, and unsafe continuations remain open.
