...
methods and all 247 property accessors now use the typed boundary (68 are
mutable), but legacy handwritten gateways and generated SwiftUI
modifier/constructor gateways still include dynamic paths.
...
| Core value algebra | Dedicated optional, array, dictionary, set, tuple, range, struct, class, enum, and opaque cases; Optional stores only `Value?` | Dedicated primitive, string, array, Set, value-backed dictionary/tuple, range, Optional, instance, enum, and host cases plus one centralized shallow/COW ownership contract; Optional additionally retains wrapped-type and IUO metadata, while dictionary lookup distinguishes its outer Optional from the nonoptional, lazily defaulted subscript | This interpreter now has the richer Optional and dictionary-default models, while SwiftScript retains the simpler physical struct/class split. Adopt a distinct/COW struct case only if profiling or API clarity justifies it. |
...
| Async execution | `eval`, statement evaluation, expression evaluation, calls, and bridges are uniformly `async throws`; async bridges may suspend, while `Task`/task-group shims run bodies inline | Dual-mode evaluator preserves synchronous rendering while `runAsync` propagates suspension through expressions, statements, user calls, task bodies, host gateways, and host-to-interpreter callbacks; each source task owns evaluator/task-local/executor/structured-scope state and host re-entry explicitly rebinds that context | This interpreter leads in covered behavior and ownership; SwiftScript's single async evaluator remains simpler. Close the bounded Task API accounting tail, then make actor identity/storage, serial executor queues, isolated hops, and reentrancy the next major runtime cycle; leave streams and continuations behind M5 resume ownership. |
| Host-call API | Bridge keys are Swift-shaped; cached `Signature` matching serves generic method candidates, while a separate hand-built `CallSignature` validator covers selected ordinary calls | `HostSignature` parses global functions, initializers, instance/static methods and properties; `HostFunction`/`HostProperty` centrally enforce labels, defaults, variadics, trailing closures, generic consistency/constraints, effects, overload ambiguity, arguments, returns, receiver types, and property writes across sync/async paths. All 115 generated Foundation method variants and 247 generated properties (68 writable) emit cached executable declarations; mutable entries carry statically compiled SDK copy mutations, and generated tags only convert already-selected method arguments for static host calls. | This interpreter leads architecturally. Continue migrating SwiftUI coverage into BridgeGen and retain handwritten code only for the narrow interface-inexpressible SwiftUI-magic primitives defined in `AGENTS.md`. |
...
| Resource safety | No general evaluator-wide step/call-depth budget or evaluator cancellation polling found in the compared revision | Fatal step/call-depth limits, bounded background slices, a 1,024-task cap, cancellation checks at evaluation boundaries, task-owned frame state, and runtime registry cleanup assertions; structured-depth/waiter/task-local limits plus stress/leak plateaus remain acceptance gaps | This interpreter leads for covered limits. Move limits into explicit session policy, bound every native/interpreted verification process, and close the machine-readable stress/resource requirements as constructs land. |
| Verification depth | 410+ feature tests and runnable parity examples | Language/integration tests plus native parity, live scenarios, and the current exact 586-project corpus gate | This interpreter leads in breadth. Every semantic migration must add focused native differential tests and keep all boards non-empty and green. |
...
2. **Typed-gateway coverage:** the executable contract layer is landed and
   ...
   complete generated Foundation method/property families use it. Move
   generated SwiftUI modifier/constructor tables off local extraction; migrate
   interface-expressible handwritten gateways into BridgeGen and retain only
   documented SwiftUI-magic primitives. Deliberate absorbing
   fallbacks may remain explicitly dynamic.
...
remains the ratcheting acceptance boundary; only a source-matching retained
receipt establishes current GREEN.
