...
| Core value algebra | Dedicated optional, array, dictionary, set, tuple, range, struct, class, enum, and opaque cases | Dedicated primitive, string, array, dictionary, tuple, range, instance, enum, and host cases | SwiftScript leads: add explicit optional/set representation where semantics require it and eliminate reference-backed core containers. |
...
1. **Value semantics:** make source structs and standard-library collections
   copy like Swift values while preserving identity for source classes,
   observable models, bindings, and host objects.
