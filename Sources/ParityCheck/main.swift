...
do {
    ...
    FileHandle.standardError.write(Data("ParityCheck: \(error)\n".utf8))
    exit(2)
}
...
if parallelOptions.shouldCoordinate, parityProbes.count > 1 {
    let jobs = min(parallelOptions.jobs, parityProbes.count)
    print("checking \(parityProbes.count) API probes with \(jobs) process shards\n")
    do {
        ...
        let diverged = try summary.required("diverged")
        let errored = try summary.required("errored")
        let unstable = try summary.required("unstable")
        let missing = try summary.required("missing")
        let total = try summary.required("total")
        guard total == parityProbes.count else {
            throw ParallelCheckError.invalidOption(
                "parity shards covered \(total)/\(parityProbes.count) probes")
        }
        print("═══ API parity: \(passed) match / \(diverged) diverge / \(errored) interp-error / \(unstable) unstable / \(missing) no-twin — of \(total) probes (\(jobs) parallel shards) ═══")
        exit(0)
    } catch {
        FileHandle.standardError.write(Data("ParityCheck: \(error)\n".utf8))
        exit(1)
...
let selectedProbes = parallelOptions.selected(from: parityProbes)
...
print("═══ API parity: \(pass) match / \(diffs.count) diverge / \(interpErrors.count) interp-error / \(unstable) unstable / \(missing) no-twin — of \(selectedProbes.count) probes ═══")
if !diffs.isEmpty {
...
if parallelOptions.shardCount > 1 {
    ParallelCheckRunner.emit(ParallelCheckSummary([
        ...
        "errored": interpErrors.count,
        "unstable": unstable,
        "missing": missing,
        "total": selectedProbes.count,
    ]))
}
